...
#define THREADED_TFT_BUF_LEN  (22)
...
static inline void write_data(const uint8_t *data, uint16_t size) {
    HAL_SPI_Transmit(&TFT_SPI, ( uint8_t *) data, size, 1);
}

static inline void write_many(uint8_t command, const uint8_t *data, uint16_t size) {
    write_command(command);
    write_data(data, size);
}

static inline void write_many_dma(uint8_t command, const uint8_t *data, uint16_t size) {
    write_command(command);
...
void tft_init(TFT_ORIENTATION orientation, uint16_t in_bg_color, uint16_t in_text_color, uint16_t in_text_color_sp,
              uint16_t in_highlight_color) { //, DISPLAY_MODE mode) {
    curr_screen = 0;
    ...
    while (tft_init_code[++i] != TFT_EOF_MARKER) {
        if (tft_init_code[i] == TFT_ORIENTATION_CMD) {
            ...
            write_many(tft_init_code[i], (const uint8_t *) &tft_init_code[i + 1], len);
            i += len + 1;
        }
    }
    ...
    for (uint8_t j = 0; j < char_max_y; j++) {
        for (uint8_t i = 0; i < char_max_x; i++) {
            if (tft_char_is_changed(tft_screens, i, j)) {
                uint8_t adj_cnt = 1;
                while ((i + adj_cnt < char_max_x) && tft_char_is_changed(tft_screens, i + (adj_cnt), j) &&
...
uint8_t tft_update2(uint32_t period) {
    ...
    switch (state) {
                ...
                for (; chari < char_max_x; chari++) {
                    if (tft_char_is_changed(tft_screens, chari, charj)) {
                        ...
                        while ((chari + adj_cnt < char_max_x) &&
                               tft_char_is_changed(tft_screens, chari + (adj_cnt), charj) &&
                               (adj_cnt < THREADED_TFT_BUF_LEN))
                            adj_cnt++; //Search for diff char
        ...
        case TFT_WAITING_DMA:
            if (tft_print_image2(&tft_dma_bufs[buf_ptr][0], chari * CHAR_WIDTH, charj * CHAR_HEIGHT,
                                 adj_cnt * CHAR_WIDTH, CHAR_HEIGHT)) {
                buf_ptr ^= 0x1;
                chari += adj_cnt;
                state = TFT_BUFFERING_PIXELS;
                return 1;
