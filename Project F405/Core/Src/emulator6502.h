...
/*	RRA
		Roll memory right one bit, then ADD the result
...
static UINT emulate6502(UINT runto)
{
	...
	while(nCPUCycle < runto)
	{
		...
		case 0xBA:	X = SP;	UpdateNZ(X);							break;	/* TSX	*/
			...
			/* ADD	*/
		case 0x61:	Ad_VlIx();	ADD();	break;
		case 0x65:	Ad_VlZp();	ADD();	break;
		case 0x69:	Ad_VlIm();	ADD();	break;
		case 0x6D:	Ad_VlAb();	ADD();	break;
		case 0x71:	Ad_VlIy();	ADD();	break;
		case 0x75:	Ad_VlZx();	ADD();	break;
		case 0x79:	Ad_VlAy();	ADD();	break;
		case 0x7D:	Ad_VlAx();	ADD();	break;
...
/*static void print_cpu_registers() {
	cout << dec << "nCPUCycle = " << nCPUCycle;
	PRINT_HEX("\tPC:", regPC);
	PRINT_HEX("\t A: ", regA);
	PRINT_HEX("\tX: ", regX);
	PRINT_HEX("\tY: ", regY);
	cout << "\tFlags: " << bitset<8>(regP);
	PRINT_HEX("\tSP:", regSP) << endl;
}

static void print_apu_registers() {
	cout << "$4000 - DDLCVVVV: " << bitset<8>(pAPU[0]) << endl;
	cout << "$4001 - EPPPNSSS: " << bitset<8>(pAPU[1]) << endl;
	cout << "$4002 - TTTTTTTT: " << bitset<8>(pAPU[2]) << endl;
	cout << "$4003 - LLLLLTTT: " << bitset<8>(pAPU[3]) << endl;

	cout << "$4004 - DDLCVVVV: " << bitset<8>(pAPU[4]) << endl;
	cout << "$4005 - EPPPNSSS: " << bitset<8>(pAPU[5]) << endl;
	cout << "$4006 - TTTTTTTT: " << bitset<8>(pAPU[6]) << endl;
	cout << "$4007 - LLLLLTTT: " << bitset<8>(pAPU[7]) << endl;

	cout << "$4008 - CRRRRRRR: " << bitset<8>(pAPU[8]) << endl;
	cout << "$4009 - --------: " << bitset<8>(pAPU[9]) << endl;
	cout << "$400A - TTTTTTTT: " << bitset<8>(pAPU[10]) << endl;
	cout << "$400B - LLLLLTTT: " << bitset<8>(pAPU[11]) << endl;

	cout << "$400C - --LCVVVV: " << bitset<8>(pAPU[12]) << endl;
	cout << "$400D - --------: " << bitset<8>(pAPU[13]) << endl;
	cout << "$400E - L---PPPP: " << bitset<8>(pAPU[14]) << endl;
	cout << "$400F - LLLLL---: " << bitset<8>(pAPU[15]) << endl;

	cout << "$4010 - IL--RRRR: " << bitset<8>(pAPU[16]) << endl;
	cout << "$4011 - -DDDDDDD: " << bitset<8>(pAPU[17]) << endl;
	cout << "$4012 - AAAAAAAA: " << bitset<8>(pAPU[18]) << endl;
	cout << "$4013 - LLLLLLLL: " << bitset<8>(pAPU[19]) << endl;

	cout << "$4015 - IF-DNT21: " << bitset<8>(pAPU[21]) << endl;
	cout << "$4017 - MI------: " << bitset<8>(pAPU[23]) << endl;
